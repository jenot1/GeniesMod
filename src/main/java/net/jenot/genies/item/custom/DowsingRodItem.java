...
import net.jenot.genies.util.InventoryUtil;
import net.jenot.genies.util.ModTags;
...
public class DowsingRodItem extends Item {
    ...
    public InteractionResult useOn(UseOnContext pContext) {
        if(pContext.getLevel().isClientSide()) {
            ...
            if(!foundBlock) {
                player.sendMessage(new TranslatableComponent("item.genies.dowsing_rod.no_valuables"),
                        player.getUUID());
            }
        }
    ...
    private void addNbtToDataTablet(Player player, BlockPos pos, Block blockBelow) {
        ...
        nbtData.putString("genies.last_ore", "Found " + blockBelow.asItem().getRegistryName().toString() + " at (" +
    ...
    public void appendHoverText(ItemStack pStack, @Nullable Level pLevel, List<Component> pTooltipComponents, TooltipFlag pIsAdvanced) {
        if(Screen.hasShiftDown()) {
            pTooltipComponents.add(new TranslatableComponent("tooltip.genies.dowsing_rod.tooltip.shift"));
        } else {
            pTooltipComponents.add(new TranslatableComponent("tooltip.genies.dowsing_rod.tooltip"));
        }
    }
